...
function getTimeSlotsFromSchedule(days, schedules, serviceDuration = 30) {
  ...
  days.forEach((day) => {
    ...
    const openingHourTotalMinutes = openingHourHours * 60 + openingHourMinutes;
    const closingHourTotalMinutes = closingHourHours * 60 + closingHourMinutes;
      ...
      if (
        ...
        timeSlotsWithAvailability[day].forEach((timeSlot) => {
          ...
          if (
            ...
            // remove timeSlot
            timeSlotsWithAvailability[day] = timeSlotsWithAvailability[
              day
            ].filter((slot) => slot.time !== timeSlot.time);
          }
        });
      }
    });
    ...
    employeeSchedules.forEach((schedule) => {
      ...
      if (
        ...
        timeSlotsWithAvailability[day].forEach((timeSlot) => {
          const dayDate = getDateFromSlotTime(day, timeSlot.time);
          dayDate.setHours(dayDate.getHours() + 2);
          if (dayDate >= scheduleStartDate && dayDate <= scheduleEndDate) {
            timeSlot.available = false;
          }
        });
      }
